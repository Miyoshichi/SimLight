...
def plot_wavefront(field, mask_r=None, plot3d=False, title=''):
    ...
    wavefront = phase(field)

    fig = plt.figure()
    if not plot3d:
        ax = fig.add_subplot(111)
        im = ax.imshow(wavefront, cmap='rainbow', extent=[-1, 1, -1, 1])
        if mask_r:
        ...
        if mask_r:
            X, Y, norm_radius = circle_aperature(wavefront, mask_r)
            max_value = np.max(wavefront[norm_radius <= mask_r])
            min_value = np.min(wavefront[norm_radius <= mask_r])
        else:
            max_value = np.max(wavefront)
            min_value = np.min(wavefront)
        stride = math.ceil(field.N / 25)
        im = ax.plot_surface(X, Y, wavefront, rstride=stride, cstride=stride,
